// lib/mediaUpload.js - VERSIONE FINALE CON PUBLIC URLs
...
export async function uploadAudio(audioBlob, interventoId, userId) {
  try {
    ...
    // ✅ USA PUBLIC URL invece di signed URL
...
export async function uploadPhoto(photoFile, interventoId, userId) {
  try {
    ...
    const cleanBlob = createCleanBlob(photoFile.file || photoFile);
...
export async function loadInterventoAllegati(interventoId) {
  console.log('🚨🚨🚨 MEDIAUPLOAD.JS VERSIONE PUBLIC URL 🚨🚨🚨');
  console.log('🚨🚨🚨 SE VEDI QUESTO MESSAGGIO IL FILE È CARICATO 🚨🚨🚨');
